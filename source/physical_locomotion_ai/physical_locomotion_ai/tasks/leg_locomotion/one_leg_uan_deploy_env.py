...
from physical_locomotion_ai.tasks.leg_locomotion.one_leg_uan_deploy_env_cfg import OneLegUANDeployableEnvCfg
from physical_locomotion_ai.uan import UANHardwareDataset


class OneLegUANDeployableCalibrationTask(DirectRLEnv):
    """Train a deployable residual actuator model from hardware transitions.

    The policy observation intentionally excludes real-sim error, so the trained
    network can later be frozen and used inside a walking simulator. During
    training, the reward still compares the simulated next state against the
    recorded hardware next state.
    """

    cfg: OneLegUANDeployableEnvCfg

    def __init__(self, cfg: OneLegUANDeployableEnvCfg, render_mode: str | None = None, **kwargs):
        expected_obs_space = cfg.history_len * cfg.per_step_observation_dim
        if cfg.observation_space != expected_obs_space:
            raise ValueError(
                "OneLegUANDeployableEnvCfg observation_space must equal "
                f"history_len * per_step_observation_dim = {expected_obs_space}, got {cfg.observation_space}."
            )
        ...
        self.prev_command = torch.zeros_like(self.actions)
        self.obs_history = torch.zeros(
            self.num_envs,
            cfg.history_len,
            cfg.per_step_observation_dim,
            dtype=torch.float32,
            device=self.device,
        )
        ...
        print(
            "[INFO] Deployable UAN dataset loaded: "
    ...
    def _apply_action(self):
        ...
        self.prev_command = command.clone()

    def _get_observations(self) -> dict:
        data_indices = self.data_start_indices + self.episode_length_buf
        command = self.dataset.cmd[data_indices]
        sim_q = self.robot.data.joint_pos[:, self.actuated_dof_indices]
        sim_dq = self.robot.data.joint_vel[:, self.actuated_dof_indices]
        command_error = command - sim_q
        command_delta = command - self.prev_command

        # All terms below are available in a walking simulator at runtime.
        step_obs = torch.cat(
            (
                command_error,
                sim_q,
        ...
        obs_extras = {
            ...
            "command_error": command_error.detach(),
            "command_delta": command_delta.detach(),
            "real_q": self.dataset.q[data_indices].detach(),
            "sim_q": sim_q.detach(),
            "real_dq": self.dataset.dq[data_indices].detach(),
    ...
    def reset_to_dataset_starts(self, starts: torch.Tensor, env_ids: torch.Tensor | None = None):
        ...
        self.prev_command[env_ids] = self.dataset.cmd[starts]
