...
- Throughout the repository, the name `physical_locomotion_ai` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of physical_locomotion_ai (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/physical_locomotion_ai
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/physical_locomotion_ai/physical_locomotion_ai/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/physical_locomotion_ai"
    ]
}
```
